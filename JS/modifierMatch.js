...
async function loadParticipations(dateMatch, heure) {
  try {
    ...
    const response = await fetchWithAuth(
      `https://drafteamapi.lespi.fr/Participation/index.php?dateMatch=${dateMatch}&heure=${heure}`,
      {
        method: "GET",
        headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Erreur HTTP! statut: ${response.status}, détail: ${errorText}`
      );
    }
...
async function modifierMatch(event) {
  ...
  try {
    ...
    const matchResponse = await fetch(
      `https://drafteamapi.lespi.fr/Match/index.php?dateMatch=${encodeURIComponent(
        originalDateMatch
      )}&heure=${encodeURIComponent(originalHeure)}`,
      {
        method: "PATCH",
        headers: {
    ...
    const participationsToDelete = existingParticipations.filter(
      (ep) =>
        !updatedParticipations.some((up) => up.numLicense === ep.numLicense)
    );
    const participationsToAdd = updatedParticipations.filter(
      (up) =>
        !existingParticipations.some((ep) => ep.numLicense === up.numLicense)
    );
    const participationsToUpdate = updatedParticipations.filter((up) =>
      existingParticipations.some(
        (ep) =>
          ep.numLicense === up.numLicense && ep.estTitulaire !== up.estTitulaire
      )
    );
    ...
    for (const participation of participationsToAdd) {
      ...
      console.log(
        "Vérification participationBody avant POST :",
        participationBody
      );
      ...
      const response = await fetch(
        `https://drafteamapi.lespi.fr/Participation/index.php?numLicense=${encodeURIComponent(
          ...
          currentDateMatch
        )}&heure=${encodeURIComponent(currentHeure)}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
