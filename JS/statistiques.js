document.addEventListener("DOMContentLoaded", () => {
    ...
    async function loadMatchStats() {
        try {
            ...
            document.querySelector('.content').innerHTML = `<p style="color: red;">Erreur lors de la récupération des statistiques : ${error.message}</p>`;
        }
    }
    ...
    async function loadPlayerStats(numLicense) {
        ...
        try {
            const response = await fetch(`https://drafteamapi.lespi.fr/Statistiques/index.php?numLicense=${numLicense}`, {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            ...
            <table class="stats-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>${(stats.pourcentageMatchsGagnes || 0).toFixed(2)}%</td>
                    </tr>
                    <tr>
    ...
    if (selectJoueur) {
        selectJoueur.addEventListener('change', (e) => loadPlayerStats(e.target.value));
    }
});
