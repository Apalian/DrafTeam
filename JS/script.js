...
function loadPlayerStats(numLicense) {
  ...
  console.log(
    `Chargement des statistiques pour le joueur avec numLicense : ${numLicense}`
  );
  ...
  fetch(`index.php?numLicense=${encodeURIComponent(numLicense)}`)
    ...
    .then((html) => {
      ...
      if (stats) {
        ...
        console.error(
          "Le conteneur #stats-container est introuvable dans la réponse."
        );
...
function logout() {
  ...
  window.location.href = "./Vue/Login.php"; // Assurez-vous que le chemin est correct
}
